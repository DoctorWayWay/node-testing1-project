...
function trimProperties(obj) {
  ...
  for (const value in trimmedObj) {
    trimmedObj[value] = trimmedObj[value].trim()
  }
  return trimmedObj
}
...
function findLargestInteger(integers) {
  ...
  integers.forEach(obj => {
    filteredArray.push(obj.integer)
  })
...
class Seasons {
  ...
  next() {
    // ✨ implement
    if (this.count === this.seasons.length) {
      this.count = 1
      return "summer"
    }
    this.count++
    return this.seasons[this.count - 1]
  }
}
